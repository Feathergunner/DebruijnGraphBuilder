...
class GraphData:
	...
	def increment_overlap(self, source_seq_id, target_seq_id, read_evidence, verbose = False):
		if target_seq_id not in self.sequences[source_seq_id].overlaps_out:
			...
			self.overlaps.append(SequenceOverlap(ov_id, self.k_value-1, source_seq_id, target_seq_id, []))
			...
			self.overlaps.append(SequenceOverlap(rev_ov_id, self.k_value-1, source_rev_seq_id, target_rev_seq_id, []))
		...
		while ov_index in range(len(self.overlaps)):
			if self.overlaps[ov_index].is_relevant:
				...
				if (self.sequences[source_id].is_relevant) and (len(self.sequences[source_id].overlaps_out) == 1):
					...
					target_id = ov.contig_sequence_2
					if verbose:
					...
					if (len(self.sequences[source_id].overlaps_out) == 1) and (len(self.sequences[target_id].overlaps_in) == 1):
						...
						self.overlaps[ov_index].is_relevant = False
