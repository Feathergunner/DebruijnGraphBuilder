...
class GraphData:
	...
	def remove_tips_simple(self, max_tip_weight=1, verbose=False):
		...
		while (num_of_removed_tips != 0):
			...
			for seq in self.sequences:
				if seq.is_relevant:
					...
					if (len(seq.sequence) < 2*self.k_value and seq.get_total_weight() <= max_tip_weight):
							is_tip = False
	...
	def remove_low_evidence_overlaps_until_graph_decomposes(self, relative_component_size_bound=0.05, final_remove_small_comps_fast=True, verbose=False):
		...
		#initilization:
		min_cov_evidence = 2
		# leave at least 10% of the initial overlaps and at least 100 overlaps
		min_overlap_number = max(100, len(self.overlaps)/10)
	...
	def remove_low_coverage_sequences_until_graph_decomposes(self, relative_component_size_bound=0.05, max_coverage_depth_to_remove = 50, verbose=False):
		...
		min_seq_number = max(100, len(relevant_sequences)/10)
